...
namespace Nimble.Automation.Repository.Helper
{
    public class HomeDetails: TestEngine
    {
        ...
        public double WeeklyExpected, FortnightlyExpected;
        ...
        public void ApplicationContinueBtnMobile()
        {

            _act.waitForVisibilityOfElement(WebSiteRefreshHomeLoc.ApplicationContinueBtn, 120);
            _act.click(WebSiteRefreshHomeLoc.ApplicationContinueBtn, "ApplicationContinueBtn");
        }

        public void ApplicationContinueBtn()
        {
            ...
            {
                _act.waitForVisibilityOfElement(WebSiteRefreshHomeLoc.ApplicationContinueBtn, 120);
                _act.click1(WebSiteRefreshHomeLoc.ApplicationContinueBtn, "ApplicationContinueBtn");
            }
        }
        ...
        public void ClickWarningBtn()
        {
            if (GetPlatform(_driver))
            {
                _act.waitForVisibilityOfElement(WebSiteRefreshHomeLoc.WarningButton, 120);
                _act.click(WebSiteRefreshHomeLoc.WarningButton, "WarningButton");
            }
            else
            {
                _act.waitForVisibilityOfElement(WebSiteRefreshHomeLoc.WarningButton, 120);
        ...
        public void WRRequestLoanAmount(int requestedAmount)
        {
            //select loan value from slider
            _act.waitForVisibilityOfElement(WebSiteRefreshHomeLoc.AmountSliderHandle, 20);
            IWebElement sliderEle = _driver.FindElement(WebSiteRefreshHomeLoc.AmountSliderHandle);
            _act.click1(WebSiteRefreshHomeLoc.AmountSliderHandle, "click");
        ...
        public void WRRequestLoanAmountMobile(int requestedAmount)
        {
            //select loan value from slider
            _act.waitForVisibilityOfElement(WebSiteRefreshHomeLoc.AmountSliderHandle, 20);
            IWebElement sliderEle = _driver.FindElement(WebSiteRefreshHomeLoc.AmountSliderHandle);
            _act.click(WebSiteRefreshHomeLoc.AmountSliderHandle, "click");
            ...
            if (requestedAmount < 1600)
            {
                ...
                int leftclicks = (actualvalue) / 25;
                for (int i = 1; i <= leftclicks; i++)
                    _act.click(WebSiteRefreshLoanPurposeLoc.SliderMinBtn, "click");
            }
            else if (requestedAmount > 1600 && requestedAmount < 2000)
            {
                int actualvalue = requestedAmount - 1600;
        ...
        public void WRLoanAmount(int requestedAmount)
        {
            ////select loan value from slider
            //_act.waitForVisibilityOfElement(WebSiteRefreshHomeLoc.AmountSliderHandle, 20);
            //IWebElement sliderEle = _driver.FindElement(WebSiteRefreshHomeLoc.AmountSliderHandle);
            ...
            if (requestedAmount < 1600)
            {
                ...
                string duration = _driver.FindElement(WebSiteRefreshHomeLoc.DefaultDuration).Text;
                string actualduration = duration.Replace(" weeks", string.Empty);

                int durationlength = Convert.ToInt32(actualduration);
                WRweeklyfortnightSACCcalc(requestedAmount, durationlength);
                WRAmountTermCalcNL(requestedAmount);
            }
            else if (requestedAmount > 1600 && requestedAmount <= 2000)
            {
            ...
            else if (requestedAmount > 2000)
            {
                ...
                string duration = _driver.FindElement(WebSiteRefreshHomeLoc.DefaultDuration).Text;
                string actualduration = duration.Replace(" months", string.Empty);
                int durationlength = Convert.ToInt32(actualduration);
                WRweeklyfortnightMACCcalc(requestedAmount, durationlength);
                WRTermCalcRL(requestedAmount);
        ...
        public void WRweeklyfortnightSACCcalc(int requestedAmount, double durationlength)
        {
            ...
            WeeklyExpected = Math.Round((requestedAmount * 1.2 + requestedAmount * 0.04 * weekscalc) / durationlength);
            FortnightlyExpected = Math.Round((requestedAmount * 1.2 + requestedAmount * 0.04 * NumberofFortnights) / FortnightlyCalc);

            WRCalculation(WeeklyExpected, FortnightlyExpected);
            WRAmountTermCalcNL(requestedAmount);
        }

        public void WRweeklyfortnightMACCcalc(int requestedAmount, int durationlength)
        {
            string duration = _driver.FindElement(WebSiteRefreshHomeLoc.DefaultDuration).Text;
            string actualduration = duration.Replace(" months", string.Empty);
            durationlength = Convert.ToInt32(actualduration);
            int durationamtweekly = durationlength * 4;
            int durationamtfortnightly = durationlength * 2;
            DateTime FirstRepaymentDateweekly = WRMACC_CalWeekly();
            //1
            DateTime FirstRepaymentDatefortnightly = WRMACC_CalFortnightly();
            //1-fort
            _act.selectByOptionText(WebSiteRefreshLoanCalculatorLoc.Weekly, "Week", "Click on Weekly");

            string RepaymentAmount = _driver.FindElement(WebSiteRefreshLoanCalculatorLoc.RePaymentAmount).Text;

            string Repay = RepaymentAmount.Replace("$", string.Empty);

            int RepayActual = Convert.ToInt32(Repay);

            _act.selectByOptionText(WebSiteRefreshLoanCalculatorLoc.Fortnightly, "Fortnight", "Click on Fortnight");

            string RepaymentAmount1 = _driver.FindElement(WebSiteRefreshLoanCalculatorLoc.RePaymentAmount).Text;

            string Repay1 = RepaymentAmount1.Replace("$", string.Empty);

            int RepayActual1 = Convert.ToInt32(Repay1);


            //WRCalculation(WeeklyExpected, FortnightlyExpected);
            WRLoanCalcluateSolver(requestedAmount, FirstRepaymentDateweekly, 7, RepayActual, durationamtweekly, durationamtweekly, RepayActual);
            WRLoanCalcluateSolver(requestedAmount, FirstRepaymentDatefortnightly, 14, RepayActual1, durationamtfortnightly, durationamtfortnightly, RepayActual1);
        }

        public void WRCalculation(double WeeklyExpected, double FortnightlyExpected)
        {
            _act.selectByOptionText(WebSiteRefreshLoanCalculatorLoc.Weekly, "Week", "Click on Weekly");
            ...
            Assert.AreEqual(RepayActual, WeeklyExpected);
            ...
            Assert.AreEqual(RepayActual1, FortnightlyExpected);

        }

        public DateTime WRMACC_CalWeekly()
        {
            var dateAndTime = DateTime.Now;
            var date = dateAndTime.Date;
            DateTime FirstWeeklyRepayDate = date.AddDays(7);
            FirstWeeklyRepayDate = WRGetBusinessDay(FirstWeeklyRepayDate);

            //Holidays list
            int length = WRHolidays().GetLength(0);

            for (int i = 0; i < length; i++)
            {
                string nationalHoliday = WRHolidays()[i, 1];
        ...
        public DateTime WRMACC_CalFortnightly()
        {
            var dateAndTime = DateTime.Now;
            var date = dateAndTime.Date;
            DateTime FirstFortnightlyRepayDate = date.AddDays(14);
            FirstFortnightlyRepayDate = WRGetBusinessDay(FirstFortnightlyRepayDate);
        ...
        public DateTime WRGetBusinessDay(DateTime _date)
        {
        ...
        public void WRLoanCalcluateSolver(double ApprovedAmt, DateTime Repaydate, int freq, double RepaymentAmount, int totalrepayments, int noofrepayments, double lastrepayemnt)
        {
            ...
            while (balanceamount > 1)
            {
                if (_date.Date == Repaydate.Date)
                {
                    ...
                    else
                    {
                        ...
                        Repaydate = WRNextRepaymentDate(freq, _date.Date);
                        noofrepayments--;
                    }
                }
        ...
        public DateTime WRNextRepaymentDate(int frequency, DateTime _date)
        {
            if (frequency == 7)
        ...
        public void clicktermslider()
        {
            _act.waitForVisibilityOfElement(WebSiteRefreshHomeLoc.DurationSliderHandle, 60);
            _act.click1(WebSiteRefreshHomeLoc.DurationSliderHandle, "DurationSliderHandle");
        }

        public void WRAmountTermCalcNL(int requestedAmount)
        {
            if (requestedAmount < 1600)
            {
                string duration = _driver.FindElement(WebSiteRefreshHomeLoc.DefaultDuration).Text;
                //double durationlength = Convert.ToInt32(duration);
                string minweeks = _driver.FindElement(WebSiteRefreshHomeLoc.MinimumDurationSliderValue).Text;
                string maxweeks = _driver.FindElement(WebSiteRefreshHomeLoc.MaximumDurationSliderValue).Text;

                Assert.Multiple(() =>
                {
                    Assert.That(duration, Is.EqualTo("10 weeks"));
            ...
            else if (requestedAmount > 2000)
            {
                ...
                {
                    string duration = _driver.FindElement(WebSiteRefreshHomeLoc.DefaultDuration).Text;
                    string minmonths = _driver.FindElement(WebSiteRefreshHomeLoc.MinimumDurationSliderValue).Text;
                    string maxmonths = _driver.FindElement(WebSiteRefreshHomeLoc.MaximumDurationSliderValue).Text;

                    Assert.Multiple(() =>
                    {
                        Assert.That(duration, Is.EqualTo("7 weeks"));
                ...
                else if (requestedAmount > 3000 && requestedAmount <= 4000)
                {
                    ...
                    Assert.Multiple(() =>
                    {
                        Assert.That(duration, Is.EqualTo("8 weeks"));
                        Assert.That(minmonths, Is.EqualTo("2 months"));
        ...
        public void WRTermCalcRL(int requestedAmount)
        {
            if (requestedAmount < 1600)
            {
            ...
            else if (requestedAmount > 2000)
            {
                ...
                if (requestedAmount > 2000 && requestedAmount <= 3000)
                {
                    ...
                    Assert.Multiple(() =>
                    {
                        Assert.That(duration, Is.EqualTo("7 months"));
                        Assert.That(minmonths, Is.EqualTo("2 months"));
                        Assert.That(maxmonths, Is.EqualTo("12 months"));
                    });
                }
                else if (requestedAmount > 3000 && requestedAmount <= 4000)
                {
                    ...
                    Assert.Multiple(() =>
                    {
                        Assert.That(duration, Is.EqualTo("8 months"));
                        Assert.That(minmonths, Is.EqualTo("2 months"));
                        Assert.That(maxmonths, Is.EqualTo("16 months"));
                ...
                else if (requestedAmount > 4000 && requestedAmount <= 5000)
                {
                    ...
                    Assert.Multiple(() =>
                    {
                        Assert.That(duration, Is.EqualTo("11 months"));
                        Assert.That(minmonths, Is.EqualTo("2 months"));
                        Assert.That(maxmonths, Is.EqualTo("22 months"));
                    });
                }
        ...
        public void ClickAmountSplitSliderHandle()
        {
            _act.waitForVisibilityOfElement(WebSiteRefreshHomeLoc.AmountSplitSliderHandle, 120);
            _act.click1(WebSiteRefreshHomeLoc.AmountSplitSliderHandle, "amount slider handle");
        }

        public string POLErrorMessage()
        {
            _act.waitForVisibilityOfElement(WebSiteRefreshHomeLoc.POLErrorMessage, 120);
            string errorMessage = _act.getText(WebSiteRefreshHomeLoc.POLErrorMessage, "POL Error message");
            return errorMessage;
        }

        public string VerifyAmountSplitErrorMessage()
        {
            _act.waitForVisibilityOfElement(WebSiteRefreshHomeLoc.AmountSplitErrorMsg, 120);
            string amountErrorMessage = _act.getText(WebSiteRefreshHomeLoc.AmountSplitErrorMsg, "Amount Split Error message");
            return amountErrorMessage;
        }

        public void NimbleLogoBtn()
        {

            _act.waitForVisibilityOfElement(WebSiteRefreshHomeLoc.logo, 120);
            _act.click(WebSiteRefreshHomeLoc.logo, "ApplicationContinueBtn");
        }

        public string ToManualOnUnmatchedEnabled()
        {
            _act.waitForVisibilityOfElement(By.XPath("//td[contains(text(),'SelectedAccountCheck.ToManualOnUnmatched')]/following-sibling::td[@class='value']"), 60);
            string ToManualOnUnmatchedEnable = _driver.FindElement(By.XPath("//td[contains(text(),'SelectedAccountCheck.ToManualOnUnmatched')]/following-sibling::td[@class='value']")).Text;
            return ToManualOnUnmatchedEnable;
        }
    }
}
